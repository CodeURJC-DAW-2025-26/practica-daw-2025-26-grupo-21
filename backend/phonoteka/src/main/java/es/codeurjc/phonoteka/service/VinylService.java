...
public class VinylService {
    ...
    private static final String[] IMAGE_SEEDS = {
    ...
    public Map<String, Object> getVinylDetails(int id) {
        int index = Math.floorMod(id - 1, TITLES.length);
        String title = TITLES[index];
        String artist = ARTISTS[index];
        String genre = GENRES[index];
        String sellerName = SELLERS[index];

        return Map.of(
            "image", "https://picsum.photos/seed/" + IMAGE_SEEDS[index] + "/600/600",
            "alt", "Portada de " + title,
            "title", title,
            "artist", artist,
            "description", "Un álbum destacado de " + artist + " dentro del " + genre.toLowerCase()
                + ". Es una pieza imprescindible para cualquier colección de vinylos.",
            "price", PRICES[index],
            "seller", seller(sellerName, "https://i.pravatar.cc/80?u=" + sellerName,
                "/user/profile", stars(4, true)));
    }

    public List<Map<String, Object>> getFeaturedVinyls(boolean logged) {
        return List.of(
    ...
    public List<Map<String, Object>> searchVinyls(String query) {
        return List.of(
    ...
    public List<Map<String, Object>> getSimilarVinyls(boolean logged) {
        return List.of(
            vinylCard("Wish You Were Here", "Pink Floyd", "Rock", "32,00 €",
